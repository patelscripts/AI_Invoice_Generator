...
const CreateInVoices = ({existingInvoice, onSave}) => {
          ...
          <tbody className='bg-white divide-y divide-slate-200'>
            {formData.items.map((item, index) => (
              <tr key={index} className='hover:bg-slate-50'>
                ...
                <td className='px-2 sm:px-6 py-4'>
                  <input type="number" name="taxPercent" value={item.taxPercent} onChange={(e) => handleInputChange(e, null, index)} className='w-full h-10 px-3 border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent' placeholder='0'/>
                </td>
    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
      ...
      <div className='bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 flex flex-col justify-center'>
        <div className='space-y-4'>
          ...
          <div className='flex justify-between text-lg font-semibold border-t border-slate-200 pt-4 mt-4 text-slate-900'><p>Total:</p><p>${total.toFixed(2)}</p></div>
        </div>
      </div>
    </div>
  </form>
