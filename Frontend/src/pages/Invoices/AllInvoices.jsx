        ...
        <div className='bg-white border border-slate-200 rounded-lg shadow-sm '>
          <div className='p-4 sm:p-6 border-b border-slate-200'>
            <div className='flex flex-col sm:flex-row gap-4'>
              <div className='relative flex-grow'>
                ...
                className='w-full h-10 pl-10 pr-4 py-2 border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-blue-500'
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <div className='flex-shrink-0'>
                <select className='w-full sm:w-auto h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm text-slate-900 focus:outline-none focus:ring-blue-500'
                value={statusFilter}
          ...
          {filteredInvoices.length  > 0  ? (
            <div className='flex flex-col items-center justify-center py-2 text-center'>
              <div className='w-16 h-16 bg-slate-100 rounded-full flex items-center justify-around mb-4'>
                      ...
                      <td onClick={() => navigate(`/invoices/${invoice._id}`)} className='px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 cursor-pointer'>{invoice.invoiceNumber}</td>
                       <td onClick={() => navigate(`/invoices/${invoice._id}`)} className='px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 cursor-pointer'>{invoice.billTo.clientName}</td>
                        <td onClick={() => navigate(`/invoices/${invoice._id}`)} className='px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 cursor-pointer'>${invoice.total.toFixed(2)}</td>
                         <td onClick={() => navigate(`/invoices/${invoice._id}`)} className='px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 cursor-pointer'>{moment(invoice.dueDate).format('MMM D, YYYY')}</td>
