...
const generateReminderEmail = async (req, res) => {
    ...
    try {
        ...
        const prompt = `You are a professional and polite accounting assistant. Write a friendly remainder email to a client about an overdue or upcoming invoice payment.
        
        Use the following details to personalize the email :
        - Client Name : ${invoice.billTo.clientName}
        - Invoice Number : ${invoice.invoiceNumber}
        - Amount Date : ${invoice.total.toFixed(2)}
        - Due Date : ${new Date(invoice.dueDate).toLocaleDateString()}

        The tone should be friendly but clear. Keep it concise. Start the email with "Subjects:"`
...
const getDashBoardSummary = async (req, res) => {
    try {
        ...
        For example , if there is a high outstanding amount, suggest sending reminders. If revenue is high, be encourging.

        Data summary : ${dataSummary}
        Return your response as a valid JSON object with a single key "insights" which is an array of Strings.
        Example format: {"insights": ["Your revenue is looking strong this months!", "You have 5 overdue invoices. Consider sending reminders to get paid faster."]}
        `
        const response = await ai.models.generateContent({
            model : "gemini-3.5-flash",
            contents:prompt,
